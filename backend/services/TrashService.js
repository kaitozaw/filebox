...
class TrashService {
    ...
    async restore(userId, fileId) {
        ...
        if (file.user.toString() !== userId) throw new ForbiddenError('Not authorized to restore this file')
    ...
    async purge(userId, fileId) {
        const file = await File.findById(fileId)
        if (!file) throw new NotFoundError('File not found')
        if (file.user.toString() !== userId) throw new ForbiddenError('Not authorized to permanently delete this file')
        if (!file.deletedAt) throw new ValidationError('File is not in trash')
